...
2. `CLAUDE.md` / `.cursor/rules/` — project conventions (if exists)
...
**Run 4 specialized analyses using subagents in parallel.**

Spawn the following 4 analyses concurrently as subagents:
...
#### Subagent 2: Architecture & Design Audit
```
...
#### Subagent 3: Code Quality & Maintainability Audit
```
...
#### Subagent 4: Testing & Reliability Audit
```
...
After gathering findings from all analyses, perform root cause analysis:
